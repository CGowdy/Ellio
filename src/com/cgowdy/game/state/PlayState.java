...
public class PlayState extends State {
	...
	public void init() {
		...
		scoreFont = new Font("SanSerif", Font.BOLD, 25);

		for (int i = 0; i < 5; i++) {
			blocks.add(new Block(i + 200, GameMain.GAME_HEIGHT - 95,
	...
	public void updateBlocks(float delta) {
		for (Block b : blocks) {
			...
			if (b.isVisible()) {
				if ((player.isDucked() && b.getRect().intersects(
						player.getDuckRect()))
						|| (!player.isDucked() && b.getRect().intersects(
								player.getRect()))) {
					b.onCollide(player);
				}
			}
